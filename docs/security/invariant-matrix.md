...
| S4 | MuSig2 secret nonces are unique, one-use, session/message-bound, consumed, and interruption-safe. | BIP327 vectors; vault MuSig2 flow verifies partials and consumes companion nonce. `MuSig2SessionSafetyTests` (8 tests) adds the refusals: reuse across messages and repeat of the same message, consumption on the throwing path so a cancelled or rolled-back attempt cannot resurrect a nonce, signer binding, cross-session replay, participant interchange, and concurrent sessions with distinct nonces. Each was observed failing under two deliberate mutations of `partialSign` before being accepted. | Remaining: duplicated participant in aggregation; crash at every round boundary and PSBT persistence rollback at the app layer, where the secret nonce is caller-held rather than stored in the PSBT. | Partial; blocked release |
